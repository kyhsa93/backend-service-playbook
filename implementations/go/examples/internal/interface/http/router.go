...
// variables" principle.
func NewRouter(repo account.Repository, cardRepo card.Repository, credentialRepo credential.Repository, paymentStore PaymentStore, accountAdapter command.AccountAdapter, paymentCardAdapter command.PaymentCardAdapter, paymentAccountAdapter command.PaymentAccountAdapter, jwtService tokenService, passwordHasher command.PasswordHasher, refundReasonClassifier command.RefundReasonClassifier, fraudRiskScorer command.RefundFraudRiskScorer, limiter *rate.Limiter, txManager command.TransactionManager) (http.Handler, *HealthHandler) {
	...
	requestRefundHandler := command.NewRequestRefundHandler(paymentStore, paymentStore, refundReasonClassifier, fraudRiskScorer)
