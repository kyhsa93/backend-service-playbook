...
func TestRequestRefundHandler_Handle_PaymentNotFound(t *testing.T) {
	...
	handler := command.NewRequestRefundHandler(store, store, &stubRefundFraudRiskScorer{})
...
// verifies that this Handler calls command.RefundFraudRiskScorer (a
// Technical Service, mocked here — never a real ML model call), and that a
// high-risk score flows through to a REJECTED refund exactly like the other
...
func TestRequestRefundHandler_Handle_RejectsWhenFraudRiskScorerFlagsHighRisk(t *testing.T) {
	...
	handler := command.NewRequestRefundHandler(store, store, scorer)
