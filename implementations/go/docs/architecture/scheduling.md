...
[domain-events.md](domain-events.md)의 구분을 그대로 따른다 — "사실이 일어났다"(Domain Event, Aggregate가 생성)와 "이 작업을 수행하라"(Task, Scheduler/Application이 생성)는 다른 개념이다. 이 저장소의 이메일 알림(Outbox + `application/event/*_event_handler.go`)은 Domain Event 소비자에 해당하고, 아직 구현되지 않은 "만료 계좌 정리 배치" 같은 것이 Task Queue의 전형적인 사용처다.
