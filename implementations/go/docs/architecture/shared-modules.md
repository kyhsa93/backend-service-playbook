...
- **`internal/infrastructure/outbox/`** — the single `outbox.Poller`/`outbox.Consumer` that `main.go` assembles share one `map[string][]outbox.Handler` across every domain (see [domain-events.md](domain-events.md)). Outbox rows are written per-BC as needed: Account's Repository takes the shared `outbox.Writer` (whose `SaveAll` signature is fixed to `[]account.DomainEvent`), Payment's Repository inserts its own outbox rows inline within its save transaction (`insertOutboxEvents` in `payment_repository.go`, including the same `trace_parent` capture as `Writer`), and Card writes no outbox rows at all — the Card Aggregate raises no Domain Events.
