...
- **`internal/infrastructure/database/`** — 여러 Repository 저장을 하나의 DB 트랜잭션으로 묶는 `WithTx`/`TxFromContext`/`QuerierFrom`/`Manager`. 계좌 간 송금(Transfer)이 출금 계좌+입금 계좌 저장을 하나의 트랜잭션으로 묶어야 했던 것이 실사용처다([persistence.md](persistence.md) 참고).
...
- **공유 패키지는 실제로 두 도메인(또는 두 Aggregate 인스턴스) 이상이 필요로 할 때만 만든다** — 미리 빈 공유 패키지를 만들어두지 않는다. `internal/common/`/`internal/config/`는 Account/Card/Credential이 실제로 공유하게 되어 만들어졌고, `internal/infrastructure/database/`(공유 트랜잭션 헬퍼)는 계좌 간 송금이 실제로 그 시나리오를 만들어서 추가됐다.
