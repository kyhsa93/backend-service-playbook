...
## 현재 상태 — `internal/infrastructure/outbox/`는 실제로 존재, 나머지는 아직 없다

`internal/` 트리를 확인하면(directory-structure.md 참고), Account 도메인 하나뿐이지만 **`internal/infrastructure/outbox/`(`Writer`/`Relay`)는 이미 실제로 존재한다** — 알림 발송이 유실되면 안 되는 부가효과여서 dual-write 대신 Outbox 패턴이 먼저 필요해졌기 때문이다([domain-events.md](domain-events.md) 참고). `common/`, `database/`, `auth/` 같은 나머지 공유 패키지는 아직 없다. 다만 이 저장소의 다른 두 문서가 이미 **미래에 만들어질 `common/`의 이름과 위치를 목표 코드로 못박아 두었다**:
...
- [directory-structure.md](directory-structure.md) — 공용 인프라 디렉토리 현황(`outbox/`는 이미 있음, `common/`/`database/`는 아직 없는 이유)
