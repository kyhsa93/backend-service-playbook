...
**LLM-backed Technical Services get a fake Ollama, not a bypass.** The four Ollama-backed services in `internal/infrastructure/llm` (the NL query translator/answer composer behind `/transactions/ask`, the transaction auto-categorizer, and the refund-reason classifier) are pointed at an in-process `httptest.Server` (`test/fake_ollama_test.go`) that emulates `POST /api/chat` and picks a deterministic response by recognizing each service's system prompt (and markers in the user message — e.g. a Starbucks merchant always classifies as FOOD). This keeps each service's real HTTP request → response-parse path in the E2E loop without a live model, so assertions can pin the exact LLM-derived outcome (the DEPOSIT-filtered grounding in the ask answer, the FOOD category on the transaction row, the DEFECTIVE_PRODUCT refund reason) instead of only the shape of a fallback. Each feature also keeps one subtest where the input embeds `forceLLMFailureMarker`, making the fake answer 500 for exactly that request — covering the graceful fallbacks (no filter + templated answer, OTHER category) the same suite used to rely on exclusively.
