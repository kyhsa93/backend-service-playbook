...
- 공개 함수/메서드: `PascalCase` — `New`, `Deposit`, `FindAccounts`, `Handle`
...
```go
// FindAccounts의 동적 필터 — zero value가 "조건 없음"을 의미
if q.AccountID != "" {
...
- 목록/단건 조회: `Find<Noun>s(ctx, query) ([]*T, int, error)` — 단건은 `query`에 ID 필터 + `Take: 1`을 담아 같은 메서드로 조회한다.
- 저장(생성/수정 겸용, upsert): `Save<Noun>(ctx, *T) error` — Noun 없이 그냥 `Save`로 두지 않는다(어떤 Aggregate를 저장하는지 메서드 이름만으로 알 수 있어야 하고, 한 패키지에 Repository가 여럿이면(예: Payment BC의 `Repository`/`RefundRepository`) 이름이 충돌한다)
- `update<Noun>` 성격의 메서드는 두지 않는다 — 상태 변경은 항상 Aggregate 도메인 메서드 호출 후 `Save`로 반영한다.

루트 [conventions.md](../../../docs/conventions.md) 섹션 5의 `find<Noun>s` 통일 규칙을 그대로 따른다. 단건 조회 호출부의 반복 패턴(첫 결과 꺼내기, 없으면 `ErrNotFound`)은 `internal/domain/account/repository.go`의 `FindOne(ctx, q, accountID, ownerID)` 같은 자유 함수로 추출해 재사용한다 — java/kotlin-springboot의 `findAccounts(...).stream().findFirst().orElseThrow(...)`와 동일한 역할이다.
