...
- **The expiry check stays on `Instant`**: a TTL is elapsed time, and an `Instant` is an absolute point on the timeline that no host timezone can move — this reading is deliberately outside the UTC-clock rule that governs persisted timestamps (see conventions.md, "the timezone rule").
