...
com.example.accountservice/
  ...
  account/
    ...
    application/
      command/                           # 쓰기 유스케이스
        ...
        PayInterestCommand.java          # record — 정기 이자 지급 배치(scheduling.md Feature 1)
        PayInterestService.java          # @Service — Task Controller가 호출하는 시스템 유스케이스
      query/                             # 읽기 유스케이스
    ...
    infrastructure/
      ...
      scheduling/                        # scheduling.md 참고
        InterestPaymentScheduler.java    # @Component @Scheduled(cron) — 매일 새벽 3시, task_outbox에 적재만 함
    ...
    interfaces/
      ...
      task/                              # Task Queue 입력 어댑터 (scheduling.md 참고)
        PayInterestTaskController.java   # taskqueue.TaskHandler 구현 — PayInterestService에 위임만
  ...
  card/                                  # 2번째 도메인 — Account BC와 Integration Event로 통신
    ...
    application/
      command/
        ...
        SendCardStatementCommand.java       # record — 월간 카드 사용내역 발송 배치(scheduling.md Feature 2)
        SendCardStatementService.java       # @Service — Task Controller가 호출하는 시스템 유스케이스
      query/
        CardQuery.java
        GetCardService.java
        GetCardResult.java
      ...
      adapter/                           # 다른 BC 조회용 ACL 인터페이스 (cross-domain.md 참고)
        AccountAdapter.java
        PaymentAdapter.java              # Payment BC 사용내역 통계 조회 — SendCardStatementService가 사용
      service/                           # 도메인 스코프 Technical Service — card만 사용
        NotificationService.java         # 인터페이스 — account/application/service/NotificationService와 동일 이유
    infrastructure/
      AccountAdapterImpl.java            # AccountAdapter 구현 — Account BC를 동기 조회
      CardPaymentAdapterImpl.java        # PaymentAdapter 구현 — Payment BC의 PaymentQuery를 동기 조회
      persistence/
        CardJpaEntity.java
        CardMapper.java
        CardJpaRepository.java
        CardRepositoryImpl.java
      notification/                      # 도메인 스코프 Technical Service 구현 — card만 사용
        CardNotificationServiceImpl.java # @Component — SES 구현체 (account/infrastructure/notification과 동일 구조)
        persistence/
          CardSentEmail.java             # @Entity — 발송 이력 (account의 SentEmail과 동일 구조, Card BC 전용)
          CardSentEmailRepository.java
      scheduling/                        # scheduling.md 참고
        CardStatementScheduler.java      # @Component @Scheduled(cron) — 매월 1일 새벽 4시, task_outbox에 적재만 함
    interfaces/
      rest/
        CardController.java
        IssueCardRequest.java
      task/                              # Task Queue 입력 어댑터 (scheduling.md 참고)
        SendCardStatementTaskController.java  # taskqueue.TaskHandler 구현 — SendCardStatementService에 위임만
  ...
  taskqueue/                             # 도메인 무관 공유 인프라 (shared-modules.md 참고) — outbox/와 형제, Task 전용
    TaskOutboxEntry.java                 # @Entity — task_outbox 테이블 (groupId/deduplicationId 포함, FIFO 큐 대응)
    TaskOutboxJpaRepository.java
    TaskHandler.java                     # taskType()별 핸들러가 구현하는 인터페이스 — 구현체는 각 도메인 interfaces/task/
    TaskOutboxWriter.java                # Scheduler(또는 Command Service)가 task_outbox 행으로 적재
    TaskOutboxPoller.java                # @Scheduled(fixedDelay=1000) — task_outbox를 폴링해 Task Queue(SQS FIFO)로 발행 (scheduling.md 참고)
    TaskConsumer.java                    # SmartLifecycle — Task Queue 수신 후 TaskHandler로 라우팅 (scheduling.md 참고)
```
...
| Task Controller | `<domain>/interfaces/task/` | `<Verb><Noun>TaskController`, `taskqueue.TaskHandler` 구현 | `PayInterestTaskController` |
