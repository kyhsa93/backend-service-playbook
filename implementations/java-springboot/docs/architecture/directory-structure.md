...
com.example.accountservice/
  ...
  outbox/                                # 도메인 무관 공유 인프라 (shared-modules.md 참고) — Account 전용 아님
    ...
    OutboxRelay.java                     # Command Service가 저장 직후 동기 호출해 드레인 (domain-events.md 참고)
```
...
알림(이메일) 발송은 `account/`와 형제인 최상위 패키지가 아니라 `account/application/service/`, `account/infrastructure/notification/`으로 **`account` 도메인 내부**에 배치되어 있다 — 실제로 `account`만 이 기능을 사용하기 때문이다(과거에는 최상위 `notification/` 패키지였으나, 최상위 공유 모듈은 여러 도메인이 실제로 공유할 때만 정당화된다는 원칙에 따라 도메인 내부로 재배치했다). [domain-service.md](../../../../docs/architecture/domain-service.md)의 Technical Service 패턴 예시를 그대로 보여준다:
...
루트가 요구하는 `common/`(ID 생성 유틸), `config/`(관심사별 설정)는 이 저장소에 아직 없다. `outbox/`(도메인 이벤트 릴레이)는 알림 발송이 유실되면 안 되는 부가효과여서 실제로 필요해져 이미 존재한다. 배치 기준:

| 디렉토리 | 포함 내용 | 상태 | 관련 문서 |
|---------|----------|------|----------|
| `common/` | `IdGenerator` 등 순수 유틸 — 프레임워크 무의존 | 아직 없음 | [aggregate-id.md](aggregate-id.md) |
| `outbox/` | `OutboxEvent`, `OutboxWriter`, `OutboxRelay`, `OutboxEventHandler` | **있음** | [domain-events.md](domain-events.md) |
| `config/` | `AwsProperties`, `SesProperties` 등 `@ConfigurationProperties` | 아직 없음 | [config.md](config.md) |

이들은 특정 도메인(`account`)에 속하지 않는 프로젝트 공용 코드이므로 도메인 패키지 밖, `com.example.accountservice` 바로 아래 최상위 패키지로 둔다 (`account/`와 같은 레벨). 반대로 `notification`처럼 실제로는 한 도메인만 사용하는 Technical Service는 공용 인프라가 아니라 그 도메인 내부(`account/infrastructure/notification/`)에 둔다 — 여러 도메인이 실제로 공유하게 되기 전까지는 최상위로 끌어올리지 않는다.
