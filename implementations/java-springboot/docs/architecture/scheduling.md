...
`examples/` has four real periodic business batches — all implement the root's `Scheduler → task_outbox → Relay → message queue → Consumer → TaskController → CommandService` path literally:
...
3. **Monthly spending-analysis ETL** (Feature 3) — the 1st of every month at 2 AM, summarizes every ACTIVE account's last month's withdrawals (total/average/%-change/trend vs. the prior month) into a `spending_analysis` read-model row, served by `GET /accounts/{accountId}/spending-analysis`. `account/infrastructure/scheduling/SpendingAnalysisScheduler` → `account/interfaces/task/AnalyzeMonthlySpendingTaskController` → `account/application/command/AnalyzeMonthlySpendingService` (idempotent via `SpendingAnalysisRepository#hasAnalysis` + a DB-level unique `(accountId, analysisMonth)` constraint).
4. **Monthly spending-forecast training** (Feature 4) — an hour after Feature 3 (3 AM on the 1st), trains a fresh linear-regression model per account on its own `spending_analysis` history (minimum 3, capped at 6 trailing months) and predicts next month's withdrawal total into a `spending_forecast` read-model row, served by `GET /accounts/{accountId}/spending-forecast`. `account/infrastructure/scheduling/SpendingForecastScheduler` → `account/interfaces/task/ForecastSpendingTaskController` → `account/application/command/ForecastSpendingService` → the `SpendingForecastModel` Technical Service (`account/infrastructure/SpendingForecastModelImpl`), idempotent via `SpendingForecastRepository#hasForecast` + a DB-level unique `(accountId, forecastMonth)` constraint. An account with fewer than 3 months of history is simply skipped and retried automatically the following month.

All four Schedulers write via `taskqueue/TaskOutboxWriter` (a shared infrastructure package, sibling to `outbox/`), which `taskqueue/TaskOutboxPoller` (`@Scheduled(fixedDelay=1000)`) drains and publishes to a Task Queue (SQS **FIFO**), and `taskqueue/TaskConsumer` (a dedicated background thread under `SmartLifecycle`) receives and invokes the `TaskHandler` registered for that `taskType` (each domain's Task Controller). This is a completely separate queue and separate table (`task_outbox`) from the Domain/Integration Event queue (`outbox/`, a standard queue) — because a Task (a command: "do X") and a Domain Event (a fact: "X happened") are conceptually different (see [domain-events.md](domain-events.md)).
