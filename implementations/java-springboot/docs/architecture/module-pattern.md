...
- **Spring Boot**: 모듈 경계는 **패키지 구조를 통한 관례(convention)**일 뿐, 컴파일러/컨테이너가 강제하지 않는다. `account` 패키지의 클래스가 `notification` 패키지의 `@Component`를 얼마든지 직접 주입받을 수 있다 — 막는 것은 코드 리뷰와 아키텍처 규율뿐이다.
...
- `SesConfig`는 `notification/infrastructure/`(도메인 소속)에 있다. 여러 도메인이 공유하는 `@Configuration`(예: 공용 `ObjectMapper` 빈)이라면 [shared-modules.md](shared-modules.md)가 다루는 공용 패키지에 두는 것이 맞다 — 이 저장소는 아직 그런 공유 설정이 없다.
...
com.example.accountservice/
  ...
  notification/       ← Technical Service (별도 BC 아님, directory-structure.md 참고)
