...
/**
 * Credential Aggregate Root — 순수 도메인 객체. 어떤 프레임워크/ORM에도 의존하지 않는다.
...
public class Credential {
    ...
    /**
     * Repository 구현체가 영속 데이터(JPA 엔티티 등)로부터 Credential을 복원할 때 사용한다.
     * create()와 달리 새 식별자를 만들지 않고 저장된 상태를 그대로 재구성한다.
     */
    ...
    /**
     * 신규 가입. 비밀번호 해싱은 Application 레이어가 PasswordHasher(Technical Service)로
     * 미리 수행한 뒤 그 결과(passwordHash)만 이 팩토리에 넘긴다 — Domain은 해싱 알고리즘을 모른다.
     */
