 ...
 * 동기 Adapter 패턴을 사용한다(cross-domain.md 참고).
 *
 * <p>반환 타입은 Account BC의 {@code AccountStatus} enum을 노출하지 않고 Card BC가 필요로 하는
 * 최소 형태({@link AccountView#active()})로 번역한다 — 상류(Account) 모델 변경이 Card 도메인으로
 * 누수되지 않게 하는 것이 ACL의 목적이다. 실제 번역은 infrastructure/AccountAdapterImpl.
 */
