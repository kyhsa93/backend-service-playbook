 ...
 * (account/infrastructure/persistence/AccountRepositoryImpl과 동일한 구조). 각 Application
 * 레이어는 자신에게 필요한 좁은 인터페이스(Repository 또는 Query)만 주입받는다.
 */
@Repository
...
public class CardRepositoryImpl implements CardRepository, CardQuery {
    ...
    public void save(Card card) {
        CardJpaEntity entity = jpaRepository.findByCardId(card.getCardId())
    ...
    public List<Card> findByAccountIdAndStatusIn(String accountId, List<CardStatus> statuses) {
        return jpaRepository.findByAccountIdAndStatusInOrderByCardIdDesc(accountId, statuses)
                .stream().map(CardMapper::toDomain).toList();
    }
