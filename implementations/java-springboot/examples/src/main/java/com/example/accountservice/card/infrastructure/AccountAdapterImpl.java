 ...
 * Account BC의 모델을 Card BC가 쓰는 최소 형태({@link AccountAdapter.AccountView})로 번역한다. Account의
 * Repository/도메인 객체를 Card의 Application·Domain 레이어로 노출하지 않는 유일한 경계 지점이다.
 *
 * <p>Account의 읽기 인터페이스는 "계좌 없음"을 예외가 아니라 {@code Optional.empty}로 표현하므로, 그 신호를 Card가 이해하는 {@code
 * Optional.empty}로 그대로 번역한다 — Account의 예외 타입이 Card 도메인으로 누수되지 않는다.
 */
@Component
