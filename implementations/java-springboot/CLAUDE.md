...
**알려진 갭**: `docs/architecture/` 각 문서는 `examples/`가 아직 반영하지 않은 항목(Aggregate ID 하이픈 제거, Query Service의 Repository 직접 사용, Repository 메서드 네이밍/`delete` 메서드 부재, 에러 응답 4필드 구조, 인증, 마이그레이션, 3계층 테스트, JPA 엔티티가 도메인을 겸하는 레이어 결합 등)을 "알려진 gap"으로 명시한다. 새 코드를 작성할 때는 `examples/`의 현재 패턴이 아니라 각 문서가 정의하는 올바른 패턴을 따른다. (Outbox 기반 이벤트 발행은 더 이상 gap이 아니다 — `outbox/` 패키지로 구현되어 있다.)
