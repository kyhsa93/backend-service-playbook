...
def test_evaluate_a_refund_at_or_under_the_payment_amount_for_a_completed_payment_is_approved() -> None:
    ...
    decision = RefundEligibilityService().evaluate(payment, refund, NOT_FRAUD, 0)
...
def test_evaluate_a_refund_classified_as_fraud_suspected_above_the_threshold_is_rejected() -> None:
    ...
    decision = RefundEligibilityService().evaluate(payment, refund, classification, 0)
...
def test_evaluate_a_refund_with_ml_fraud_risk_score_at_or_above_the_threshold_is_rejected() -> None:
    payment = make_completed_payment(amount=10000)
    refund = Refund.create(payment_id=payment.payment_id, amount=5000, reason="personal change of mind")

    decision = RefundEligibilityService().evaluate(payment, refund, NOT_FRAUD, 0.8)
...
def test_evaluate_a_refund_with_ml_fraud_risk_score_below_the_threshold_is_still_approved() -> None:
    payment = make_completed_payment(amount=10000)
    refund = Refund.create(payment_id=payment.payment_id, amount=5000, reason="personal change of mind")

    decision = RefundEligibilityService().evaluate(payment, refund, NOT_FRAUD, 0.79)
