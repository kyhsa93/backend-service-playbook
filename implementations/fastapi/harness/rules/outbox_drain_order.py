...
def check(root: str, py_files: list[str]) -> RuleResult:
    ...
    for f in py_files:
        ...
        if not save_match:
            ...
            result.add(failed(r, "OutboxRelay를 참조하지만 process_pending() 호출이 없음 — 저장 직후 Outbox 드레인 누락(domain-events.md)"))
        elif pp_match.start() < save_match.start():
            result.add(failed(r, "process_pending() 호출이 save(...) 호출보다 먼저 등장함 — 커밋 이후 드레인 순서 위반"))
        else:
