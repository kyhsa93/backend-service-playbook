...
우연히 "outbox" 문자열이 들어간 무관한 파일에 낚이지 않기 위함이다. 실제 파일이 전부
src/outbox/ 안에 있는 구조에서 "밖에 있는 파일 찾기" 조건으로는 "outbox 패턴 없음"이라는
거짓 SKIP이 날 수 있으므로 주의한다.

outbox 구성 요소는 OutboxWriter(적재)+OutboxPoller(발행)+OutboxConsumer(수신)다 —
트리거 심볼은 OutboxWriter를 쓴다.
...
`application/integration_event/`와 동일한 이유로 underscore를 쓴다.
"""
