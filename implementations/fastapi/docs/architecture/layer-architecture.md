...
Unit tests: `tests/unit/infrastructure/test_nl_transaction_query_translator_impl.py`/`test_nl_transaction_answer_composer_impl.py` mock `httpx.AsyncClient` directly (valid response parsed, invalid/malformed values dropped, network failure falls back), and `tests/unit/application/test_ask_transaction_history_handler.py` mocks the repo/translator/composer, pinning that the retrieval is always scoped by the authenticated requester regardless of what the mocked translator returns. `tests/test_account_e2e.py`'s `test_ask_transaction_history_*` cases exercise the real `POST /accounts/{account_id}/transactions/ask` endpoint end-to-end against the deterministic fake Ollama in `tests/fake_ollama.py` (a respx mock serving the real `/api/chat` request/parse path — see [testing.md](testing.md)), asserting the translated filter narrowed the retrieval and the answer was grounded in exactly the matching transactions; a dedicated case embeds the fake's failure marker in the question to force both LLM calls onto their non-blocking fallbacks.
...
The unit test also instantiates `RefundEligibilityService` directly without going through the Application layer, passing in
plain `Payment`/`Refund` values and verifying only the decision logic (`tests/unit/domain/test_refund_eligibility_service.py`).

Full code: `examples/src/payment/domain/refund_eligibility_service.py`, `payment.py`,
