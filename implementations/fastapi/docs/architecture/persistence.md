...
harness `no-orm-autosync-in-prod-config` 규칙(`../../harness/rules/no_orm_autosync_in_prod_config.py`)이 테스트 전용 파일이 아닌 곳(`main.py`, `src/database.py` 등 앱 부트스트랩 경로)에서 `Base.metadata.create_all(...)` 호출이 있는지 AST로 검사한다 — 위 "스키마 변경은 Alembic으로 관리한다" 원칙을 기계적으로 강제한다.
