...
```python
# src/common/correlation.py — 실제 코드
...
```python
# src/account/interface/rest/account_router.py — 실제 코드. 라우터 전체에 공통 의존성 적용
# (개별 라우트에서 빠뜨릴 위험 없음)
...
→ 토큰 검증 로직 자체는 [authentication.md](authentication.md) 참조.
