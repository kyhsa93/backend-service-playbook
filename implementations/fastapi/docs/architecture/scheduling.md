...
이 저장소에는 아직 스케줄링/배치 작업이 없다 (Account 도메인은 Cron이 필요한 유스케이스가 없다). 이 문서는 필요해질 때(예: 만료 계좌 정리 배치 등)를 위한 FastAPI/Python 패턴을 정의한다.

**주의**: [domain-events.md](domain-events.md)의 `src/outbox/outbox_relay.py`(`OutboxRelay.process_pending()`)는 이미 실제로 구현되어 있지만, 아래에서 다루는 APScheduler 폴링 방식이 **아니다** — Command Handler가 `repo.save()` 직후 동기 호출해 드레인하는 방식을 택했다. 아래 절은 이 알림 발송과 무관하게, 진짜 주기적 배치(만료 계좌 정리 등)가 필요해질 때 참고할 일반적인 FastAPI 패턴이다.
...
`BackgroundTasks`는 스케줄링 도구가 아니라 "응답 후 후처리" 도구다 — 예를 들어 [domain-events.md](domain-events.md)에서 다루는 알림 발송을 지금의 `OutboxRelay.process_pending()` 동기 드레인 대신 `BackgroundTasks`로 옮기면, 커밋 이후 응답 전송 전에 프로세스가 죽었을 때 유실 문제가 그대로 남는다(재시도 메커니즘이 없으므로). 재시도가 필요한 작업에는 적합하지 않다.
...
아래는 (알림용 `OutboxRelay`와는 별개로) 진짜 주기적 배치가 생길 때의 가상 예시다 — 예를 들어 만료 계좌를 주기적으로 정리하는 `cleanup_expired_accounts()`라는 Application Handler가 있다고 하자:
...
- [domain-events.md](domain-events.md) — `OutboxRelay.process_pending()`의 실제 구현(동기 드레인, 스케줄러 아님)
