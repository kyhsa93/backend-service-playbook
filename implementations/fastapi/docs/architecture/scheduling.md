...
이 저장소에는 아직 스케줄링/배치 작업이 없다 (Account 도메인은 Cron이 필요한 유스케이스가 없다). 이 문서는 필요해질 때(예: [domain-events.md](domain-events.md)의 Outbox Relay, 만료 계좌 정리 배치 등)를 위한 FastAPI/Python 패턴을 정의한다.
...
`BackgroundTasks`는 스케줄링 도구가 아니라 "응답 후 후처리" 도구다 — 예를 들어 [domain-events.md](domain-events.md)에서 다루는 알림 발송을 Outbox 폴링 대신 `BackgroundTasks`로 옮기면 프로세스가 죽었을 때 유실 문제가 그대로 남는다. 재시도가 필요한 작업에는 적합하지 않다.
...
```python
# infrastructure/scheduling/outbox_relay_scheduler.py (신설 제안)
import logging

from apscheduler.schedulers.asyncio import AsyncIOScheduler

from ..persistence.outbox_relay import relay_pending_events

logger = logging.getLogger(__name__)


def start_outbox_relay_scheduler(session_factory) -> AsyncIOScheduler:
    scheduler = AsyncIOScheduler()

    @scheduler.scheduled_job("interval", seconds=5, id="outbox-relay")
    async def _run() -> None:
        try:
            async with session_factory() as session:
                await relay_pending_events(session)
        except Exception:
            logger.exception("outbox_relay_scheduler_failed")
...
from src.account.infrastructure.scheduling.outbox_relay_scheduler import start_outbox_relay_scheduler


@asynccontextmanager
async def lifespan(_: FastAPI) -> AsyncGenerator[None, None]:
    ...
    scheduler = start_outbox_relay_scheduler(SessionLocal)
    yield
...
- [domain-events.md](domain-events.md) — Outbox Relay를 스케줄러로 실행하는 예
