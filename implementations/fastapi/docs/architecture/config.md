...
Not every `config/` file needs to be a `BaseSettings` class — `config/rate_limit_config.py` (`RATE_LIMIT_DEFAULT`, `RATE_LIMIT_WRITE`) and `config/interest_config.py` (`ACCOUNT_INTEREST_DAILY_RATE`) instead just read `os.getenv(...)` with a default in their constructor, since these values are tunable operational parameters with a safe default, not values that should halt startup if missing. `config/llm_config.py` (`get_llm_model()`/`get_ollama_base_url()`, `LLM_MODEL`/`OLLAMA_BASE_URL`) follows the same plain-function shape rather than a `BaseSettings` class, for the same reason — Ollama is self-hosted (see [local-dev.md](local-dev.md)), so there's no secret to guard and no environment where the app must refuse to start without it. It's used by `account/infrastructure/nl_transaction_query_translator_impl.py`/`nl_transaction_answer_composer_impl.py` — see [layer-architecture.md](layer-architecture.md) for the structured-data RAG pipeline these two Technical Services form.
