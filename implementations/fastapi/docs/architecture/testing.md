...
The LLM-backed features (transaction-history Q&A, transaction auto-categorization, refund-reason classification/insights) don't degrade to their graceful-fallback branch in e2e — `tests/fake_ollama.py` mounts a deterministic fake Ollama with [respx](https://lundberg.github.io/respx/) (the standard `httpx` mocker), and `tests/conftest.py` pins `OLLAMA_BASE_URL` to its fixed fake origin and keeps the router active for the whole session via a session-scoped autouse fixture — session-wide because the background `OutboxConsumer` calls Ollama asynchronously (categorization/refund-reason classification), so the mock must outlive any single test. The fake serves the one endpoint every LLM Technical Service calls, `POST /api/chat`, and routes statelessly on content: the system prompt identifies which service is calling, and the user message picks the deterministic answer (a "deposit" question → a `DEPOSIT` filter, a "Starbucks" merchant → `FOOD`, an "arrived broken" reason → `DEFECTIVE_PRODUCT`, and the composer echoes its grounding transactions back behind a recognizable prefix) — so each impl's real HTTP request/response-parse path runs and gets asserted exactly. Embedding the `force-llm-500` marker in the natural input that reaches a prompt (question/`merchant_name`/`reason`) makes the fake answer 500 for exactly that request, which is how the per-feature fallback tests keep the graceful-degradation branch covered. Only the fake Ollama origin is intercepted — a catch-all `pass_through()` route lets every other `httpx` request (and respx never touches the `ASGITransport` test clients or the aiohttp-based LocalStack traffic to begin with) reach its real transport untouched.
