...
One more namespace follows the same per-concern shape but deliberately opts out of `@Validated`
fail-fast: `fraud-scorer` (`FraudScorerProperties` — `mode`/`base-url`, for
`payment/infrastructure/RefundFraudRiskScorerNativeImpl.kt`/`RefundFraudRiskScorerHttpImpl.kt`).
It already has sane defaults (`native`/`http://localhost:8000`), and a missing/blank value must
never fail application startup — an ML scoring outage is tolerated at runtime as a non-blocking
fallback (see each impl's `catch`/fallback-score handling), not a fail-fast condition the way
`aws`/`ses`/`jwt` are. `fraud-scorer.mode` additionally drives bean selection: each of
