...
`examples/src/main/resources/application.yml`에는 `server.shutdown` 설정이 없다. 기본값(`immediate`)에서는 SIGTERM 수신 즉시 진행 중인 요청을 끊고 종료한다. 아래는 Spring Boot 내장 기능만으로 root 원칙을 충족하는 방법이다 — 별도 라이브러리가 필요 없다.

---

## `server.shutdown: graceful` — 한 줄로 얻는 것
...
## Liveness / Readiness — Spring Boot Actuator

[container.md](container.md)에서 다룬 Actuator 헬스 프로브가 여기서도 핵심이다.
