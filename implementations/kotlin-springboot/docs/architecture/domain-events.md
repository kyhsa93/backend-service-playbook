...
이 저장소의 `examples/`는 이 규칙을 그대로 구현한다 — Command Service는 저장 커밋 직후 같은
프로세스 안에서 동기적으로 드레인하지 않는다. `OutboxPoller`가 독립적으로 1초 주기로 Outbox
테이블을 SQS로 발행하고, `OutboxConsumer`가 SQS를 수신 대기하다가 `EventHandlerRegistry`를
통해 핸들러를 호출한다.
...
`processed`는 **"`OutboxPoller`가 SQS로 발행을 끝냈다"**는 뜻이다. 그 이후의 재시도/at-least-once
보장은 이 컬럼이 아니라 SQS의 visibility timeout + DLQ가 담당한다.
...
## 3단계: `EventHandlerRegistry` — 핸들러 라우팅
...
생성자 실행 시점에 `Map<String, (eventId, payload) -> Unit>` 하나로 미리 구성해 두고
`OutboxConsumer`가 조회하는 방식이다. Kotlin/Spring이 자동 수집하는 `List<Interface>` 패턴
(java-springboot의 방식)이 아니라 생성자에 각 핸들러를 개별 파라미터로 주입받으므로, 새 도메인을
추가할 때 이 파일을 직접 고쳐야 한다(스캐폴딩 생성기의 `--wire`가 이를 자동화한다).
...
## 4단계: `OutboxPoller` — Outbox → SQS 전송
...
## 5단계: `OutboxConsumer` — SQS → EventHandler 수신
...
핸들러 자체의 코드는 호출 경로(`OutboxConsumer` → `EventHandlerRegistry`)와 무관하다 — 이벤트
타입별 처리 로직만 담고 있으면 된다.
