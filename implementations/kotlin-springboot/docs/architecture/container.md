...
- harness `dockerfile-conventions` 규칙이 `examples/Dockerfile`을 텍스트로 파싱해 위 원칙 중 기계적으로 검증 가능한 네 가지 — 멀티스테이지 빌드(`FROM` 2개 이상), `HEALTHCHECK` 존재, `USER` 존재(non-root 실행), `.dockerignore`의 빌드 산출물(`build/`, `.gradle/` 등)/`.git` 제외 패턴 존재 — 를 확인한다.
