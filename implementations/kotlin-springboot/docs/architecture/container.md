...
## 적용 완료 — Dockerfile (Actuator 헬스체크는 남은 갭)

`implementations/kotlin-springboot/examples/Dockerfile`은 아래 멀티스테이지 빌드를 실제 코드 그대로 구현한다(Gradle Kotlin DSL 빌드, Kotlin 1.9.25 + Spring Boot 3.3.5 + Java 21 toolchain). `.dockerignore`도 아래 내용 그대로 존재한다. **아직 남은 갭은 Actuator 기반 liveness/readiness 프로브뿐이다** — `build.gradle.kts`에 `spring-boot-starter-actuator` 의존성이 없고 `application.yml`에도 `management.*` 설정이 없다(아래 "헬스체크 엔드포인트" 절 참고).
...
## 헬스체크 엔드포인트 — Spring Boot Actuator (남은 갭, 아직 미적용)

Spring Boot는 Actuator만 추가하면 liveness/readiness 프로브를 별도 컨트롤러 없이 얻는다. `examples/build.gradle.kts`에는 아직 `spring-boot-starter-actuator`가 없다 — `SecurityConfig`가 화이트리스트로 열어둔 `/health/**`도 현재는 Actuator가 아니라 별도 엔드포인트를 전제로 한 경로다. 아래는 도입 시 따를 방향성이다.
...
- **멀티스테이지 빌드 필수(적용 완료)**: `examples/Dockerfile`이 Gradle/Kotlin 컴파일러와 전체 소스를 빌드 스테이지에만 두고 최종 이미지에는 포함하지 않는다.
- **JRE 베이스 이미지 사용(적용 완료)**: 런타임 스테이지는 `eclipse-temurin:21-jre-alpine`.
- **.dockerignore로 `.gradle/`, `build/` 제외(적용 완료)**.
- **CMD는 exec form(적용 완료)**: `["java", "-jar", "app.jar"]`.
- **환경 변수는 이미지 외부에서 주입(적용 완료)**: `Dockerfile`에 `ENV`로 값을 굽지 않는다.
- **Actuator 기반 liveness/readiness 프로브 — 남은 갭**: `build.gradle.kts`에 Actuator 의존성 없음, `/health/**`는 아직 실제로 서빙되지 않는 경로.
