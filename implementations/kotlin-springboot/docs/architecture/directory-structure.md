...
이 예제는 단일 BC(Account) 구조라 root가 규정하는 공용 패키지 대부분이 아직 없다. `outbox/`는 예외다 — 알림 발송이 유실되면 안 되는 부가효과여서 실제로 필요해져 이미 추가되었다. 도메인이 더 늘어나거나 비동기 처리가 필요해지면 나머지도 프로젝트 루트(`com.example.accountservice.*`, `account/` 밖)에 추가한다.
