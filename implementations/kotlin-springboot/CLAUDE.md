...
| 작업 / 키워드 | 읽을 문서 |
|---------------|----------|
| `@Scheduled`, Outbox Poller/Consumer, 코루틴 미사용 이유 | `docs/architecture/scheduling.md` |
...
Outbox 기반 이벤트는 `outbox/`, `account/application/event/*EventHandler.kt`로 구현되어 있다. 새 코드를 작성할 때는 `examples/`의 현재 패턴이 아니라 각 문서가 정의하는 올바른 패턴을 따른다.
