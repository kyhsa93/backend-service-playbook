...
**알려진 갭**: `docs/architecture/` 각 문서는 `examples/`가 아직 반영하지 않은 항목(Aggregate ID 하이픈 제거, Outbox 기반 이벤트, Repository 메서드 네이밍, 에러 응답 4필드 구조, 인증, 마이그레이션, 3계층 테스트 등)을 "알려진 갭"으로 명시한다. 새 코드를 작성할 때는 `examples/`의 현재 패턴이 아니라 각 문서가 정의하는 올바른 패턴을 따른다.
