 ...
 * classifies a refund's free-text reason. [com.example.accountservice.payment.domain.RefundEligibilityService]
 * (a Domain Service) never depends on this interface and never calls an LLM itself — it only ever
 * receives the already-computed [RefundReasonClassification] as a plain value and makes its own pure
...
interface RefundReasonClassifier {
    fun classify(reason: String): RefundReasonClassification
}
