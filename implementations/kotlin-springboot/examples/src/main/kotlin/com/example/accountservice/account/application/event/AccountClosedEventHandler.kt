...
/**
 * Sends an account-closure notification email when AccountClosedEvent is delivered via the Outbox, and
 * writes an Integration Event (account.closed.v1) notifying external BCs (Card, etc.) into the Outbox.
 *
 ...
 * [com.example.accountservice.outbox.OutboxConsumer] invokes when it receives AccountClosedEvent from
...
class AccountClosedEventHandler(
    ...
    fun handle(
        ...
        // The notification is best-effort (same reason as the suspend handler — if this handler is
        // allowed to throw on failure, the Integration Event already written would be written again on
        // the next re-drain). Retrying the notification itself is the responsibility of a separate
        // outbox row (the sent_email pipeline). eventId is the eventId of this Outbox row —
        // NotificationService uses it as the key to apply Level 2 (Ledger) duplicate-send prevention.
        try {
            ...
            logger
                ...
                .setCause(e)
                .log("Failed to send closure notification")
        }
    }
}
