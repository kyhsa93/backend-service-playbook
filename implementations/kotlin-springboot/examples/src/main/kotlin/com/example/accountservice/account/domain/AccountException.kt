...
class InvalidAmountException :
    AccountException("금액은 0보다 커야 합니다.", AccountErrorCode.INVALID_AMOUNT)
class InvalidMoneyAmountException :
    AccountException("금액은 0 이상이어야 합니다.", AccountErrorCode.INVALID_MONEY_AMOUNT)
class CurrencyMismatchException :
    AccountException("통화가 일치하지 않습니다.", AccountErrorCode.CURRENCY_MISMATCH)
...
class AccountBalanceNotZeroException :
    AccountException("잔액이 0이 아닌 계좌는 종료할 수 없습니다.", AccountErrorCode.ACCOUNT_BALANCE_NOT_ZERO)
class DeleteRequiresClosedAccountException :
    AccountException("종료 상태의 계좌만 삭제할 수 있습니다.", AccountErrorCode.DELETE_REQUIRES_CLOSED_ACCOUNT)
