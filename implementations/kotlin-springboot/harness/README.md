...
| `utc-timestamp-source` | `UtcTimestampSource.kt` | fails on a bare `LocalDateTime.now()`/`LocalDate.now()` (the no-argument form) in `domain/` or `infrastructure/persistence/` — it resolves against the JVM's default zone, so the wall clock written into a `TIMESTAMP` (WITHOUT TIME ZONE) column depends on the machine the process runs on, one column ends up holding values that cannot be compared, and period arithmetic built on them shifts by the offset (`LocalDate.now()` additionally names the wrong *day* near midnight, changing period keys like `"2026-08"`). `LocalDateTime.now(ZoneOffset.UTC)` and a shared helper wrapping it both pass; comments are stripped before matching, so a bare call quoted in prose is not counted. The scope is deliberately limited to those two package kinds: a clock reading there is by construction a value that gets persisted or shipped inside a domain event, whereas legitimate elapsed-time measurement (a request-latency stopwatch in a Filter/HandlerInterceptor, TTL-cache and retry-backoff deadlines, JWT `iat`/`exp`, which serializes as a location-free epoch number) lives outside it and is never flagged — the timezone rule in `docs/conventions.md` |
