...
이 저장소에는 커스텀 `HttpExceptionFilter`가 없다. `generateErrorResponse`가 이미 `{ statusCode, code, message, error }` 형태의 응답 객체를 담은 `HttpException`을 만들어 던지므로, NestJS 기본 예외 처리가 `exception.getResponse()`를 그대로 직렬화하는 것만으로 충분하다. 응답 로깅까지 표준화하고 싶다면 아래 패턴을 추가한다.

```typescript
// src/common/http-exception.filter.ts — 필요시 추가할 확장 패턴, 현재 코드에는 없음
