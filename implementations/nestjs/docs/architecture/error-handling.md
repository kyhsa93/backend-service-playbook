...
이 저장소에는 커스텀 `HttpExceptionFilter`가 `app.useGlobalFilters(...)`로 등록되어 있다([bootstrap.md](bootstrap.md) 참고). `generateErrorResponse`가 만든 `HttpException`은 이미 `{ statusCode, code, message, error }` 형태의 응답 객체를 담고 있으므로 그대로 직렬화한다. 이 필터의 핵심 역할은 `HttpException`이 아닌 미처리 예외(예: 예상치 못한 `Error` throw)까지 잡아 표준 에러 응답 형식으로 변환하는 것이다 — 그렇지 않으면 NestJS 기본 처리기가 스택트레이스를 포함한 원시 500 응답을 그대로 노출한다.

```typescript
// src/common/http-exception.filter.ts — 실제 코드
