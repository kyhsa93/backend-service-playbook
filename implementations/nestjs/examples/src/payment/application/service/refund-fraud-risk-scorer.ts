...
// fraud-risk model — trained on the requester's own structured refund/payment history (refund
// count, rejection count, amount ratio, minutes since payment), not user-supplied free text.
// RefundEligibilityService never depends on this interface and never trains or calls a model
// itself — it only ever receives the already-computed score as a plain number and applies its
// own fixed threshold. Two implementations exist side by side
