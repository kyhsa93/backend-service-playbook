...
// The app-object-level setup shared between the production entrypoint (src/main.ts) and every
// E2E spec (test/support/test-app.ts). Both run the exact same pipeline — security headers,
// global ValidationPipe, interceptors, exception filter, CORS, shutdown hooks — so a test never
// exercises a lookalike assembly that can drift from what production actually runs. main.ts
// keeps only the parts that make no sense for a test app: Swagger document serving and listen().
