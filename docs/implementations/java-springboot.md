...
| [cqrs-pattern.md](../architecture/cqrs-pattern.md) | **Covered (코드는 일부 gap)** | `docs/architecture/cqrs-pattern.md` | 현재의 경량 CQRS(Command/Query Service 분리)를 문서화하고, `AccountQueryRepository` 인터페이스 도입(올바른 패턴)과 Handler 기반 CQRS 전환 기준을 명시. **`GetAccountService`는 이제 쓰기용 `AccountRepository`가 아니라 좁은 `AccountQueryRepository`(application/query)에 의존한다** — `AccountRepositoryImpl`이 두 인터페이스를 모두 구현해 DI를 분기한다. 남은 gap: `GetTransactionsService`는 아직 `findTransactions`/`countTransactions`가 `AccountQueryRepository`에 없어 쓰기용 `AccountRepository`를 그대로 사용한다. |
...
**요약**: 24개 root 아키텍처 문서 중 **Covered 21 (그중 완전히 해소된 항목 8, 일부만 남은 항목 6, 코드 gap이 여전히 남아있는 항목 1 — 어떤 형태로든 gap이 있는 항목 합계 7) / Thin 1 / Missing 2**. 이전 감사(Covered 4 / Thin 9 / Missing 11) 대비 문서 커버리지가 크게 확장되었고, JWT 인증·Correlation ID/요청 로깅·Secrets Manager·aggregate-id·테스트 피라미드 5개 항목은 이전 세션에서 코드가 문서를 따라잡아 gap 목록에서 완전히 빠졌다(`domain-events.md`는 그보다 앞서 이미 빠졌다). 이번 세션에서는 `layer-architecture`(Domain/JPA 분리 — `AccountJpaEntity`/`TransactionJpaEntity`/`MoneyEmbeddable` + `AccountMapper`/`TransactionMapper` 도입)와 `error-handling`(`ErrorResponse` 4필드화)이 완전히 해소되었고, `repository-pattern`(`AccountRepository.delete()` soft delete 추가)과 `cqrs-pattern`(`GetAccountService`의 `AccountQueryRepository` 분리)은 핵심 gap이 해소되고 지엽적인 gap만 남았다(각각 단건 조회 메서드 네이밍, `GetTransactionsService`의 Query 인터페이스 미전환). config/observability/persistence/container 4개 항목도 핵심 gap이 해소되고 지엽적인 gap만 남았다. 남은 완전 gap은 `graceful-shutdown`(Actuator 미도입) 하나뿐이다.
