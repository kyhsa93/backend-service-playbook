...
**요약**: 24개 root 아키텍처 문서 중 **Covered 21 (그중 완전히 해소된 항목 9, 일부만 남은 항목 5, 코드 gap이 여전히 남아있는 항목 1 — 어떤 형태로든 gap이 있는 항목 합계 6) / Thin 1 / Missing 2**. 이전 감사(Covered 4 / Thin 9 / Missing 11) 대비 문서 커버리지가 크게 확장되었고, JWT 인증·Correlation ID/요청 로깅·Secrets Manager·aggregate-id·테스트 피라미드 5개 항목은 이전 세션에서 코드가 문서를 따라잡아 gap 목록에서 완전히 빠졌다(`domain-events.md`는 그보다 앞서 이미 빠졌다). 이후 세션에서는 `layer-architecture`(Domain/JPA 분리 — `AccountJpaEntity`/`TransactionJpaEntity`/`MoneyEmbeddable` + `AccountMapper`/`TransactionMapper` 도입), `error-handling`(`ErrorResponse` 4필드화), `cqrs-pattern`(`GetTransactionsService`도 `AccountQuery`로 전환 + `AccountQueryRepository` → `AccountQuery` 리네이밍 + harness `cqrs-query-purity` 규칙 추가)이 완전히 해소되었고, `repository-pattern`(`AccountRepository.delete()` soft delete 추가)은 핵심 gap이 해소되고 지엽적인 gap(단건 조회 메서드 네이밍)만 남았다. config/observability/persistence/container 4개 항목도 핵심 gap이 해소되고 지엽적인 gap만 남았다. `notification`은 최상위 공유 모듈에서 `account` 도메인 내부(Technical Service)로 재배치되었다. 남은 완전 gap은 `graceful-shutdown`(Actuator 미도입) 하나뿐이다.
...
| `docs/architecture/design-principles.md` | 이 저장소의 핵심 설계 원칙 13개를 압축한 TL;DR 인덱스 + 남은 gap(`GetTransactionsService`의 Query 인터페이스 미전환 등) 요약 표 |
