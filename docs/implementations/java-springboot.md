...
| [repository-pattern.md](../architecture/repository-pattern.md) | **Covered (코드는 일부 gap)** | `docs/architecture/repository-pattern.md` | 올바른 `find<Noun>s`/`save<Noun>`/`delete<Noun>` 컨벤션과 `take:1` 단건 조회 패턴을 정의. **`AccountRepository`에 `delete(String accountId)`(soft delete)가 추가되어 root의 "delete 메서드 필요" 원칙과 이제 일치한다.** 남은 gap: `findByAccountIdAndOwnerId`가 여전히 단건 전용 별도 메서드다(`findAll`+`take:1`로 통일하지 않은 실용적 트레이드오프). |
