...
| [container.md](../architecture/container.md) | **Covered (코드는 gap)** | `docs/architecture/container.md` | Layered JAR 기반 Gradle 멀티스테이지 Dockerfile, JRE 베이스, Actuator 헬스체크를 정의. **`examples/Dockerfile`이 실제로 존재하지만(멀티스테이지, Layered JAR), `container.md` 문서 자체가 아직 "Dockerfile 없음"으로 서술되어 있어 문서와 코드가 불일치한다** — `container.md`는 이번 문서화 패스 범위 밖이라 갱신하지 않았다(후속 이슈로 트래킹). |
...
**요약**: 24개 root 아키텍처 문서 중 **Covered 21 (그중 완전히 해소된 항목 4, 일부만 남은 항목 3, 코드 gap이 여전히 남아있는 항목 7 — 어떤 형태로든 gap이 있는 항목 합계 10) / Thin 1 / Missing 2**. 이전 감사(Covered 4 / Thin 9 / Missing 11) 대비 문서 커버리지가 크게 확장되었고, JWT 인증·Correlation ID/요청 로깅·Secrets Manager·aggregate-id 4개 항목은 이번 세션에서 코드가 문서를 따라잡아 gap 목록에서 완전히 빠졌다(`domain-events.md`는 이전 세션에서 이미 빠짐). config/observability/persistence 3개 항목은 핵심 gap(fail-fast 검증, 구조화 로깅, 마이그레이션)이 해소되고 지엽적인 gap만 남았다. 남은 7개 완전 gap: `layer-architecture`(Domain의 JPA 결합), `repository-pattern`(메서드 네이밍/`delete` 부재), `error-handling`(에러 응답 2필드), `testing`(3계층 테스트), `cqrs-pattern`(Query의 Repository 직접 사용), `container`, `graceful-shutdown`(Actuator 미도입).

**참고 — 이번 패스 범위 밖에서 추가로 발견한 문서-코드 불일치**: `testing.md`(Domain/Application 단위 테스트가 실제로는 `AccountTest`/`MoneyTest`/`CreateAccountServiceTest`/`DepositServiceTest`로 이미 존재함에도 문서는 "없음"으로 서술)와 `container.md`(`examples/Dockerfile`이 실제로 존재함에도 문서는 "Dockerfile 없음"으로 서술)는 이번 이슈(#103)의 감사 대상에 포함되지 않아 로컬 문서 자체를 갱신하지 않았다 — 위 표의 해당 행은 로컬 문서 내용과 일치시키기 위해 의도적으로 기존 gap 서술을 유지했다. 후속 이슈로 갱신을 트래킹한다.
