...
| [observability.md](../architecture/observability.md) | **Covered** | `docs/architecture/observability.md` | 구조화 로깅(Logback JSON 인코더 + `StructuredArguments.kv`), MDC 기반 Correlation ID, Actuator/Micrometer 메트릭을 정의. `NotificationServiceImpl`/`OutboxRelay`/`AccountController` 모두 `StructuredArguments.kv(...)`로 구조화 로깅을 하고, `logback-spring.xml`(로컬 평문/운영 JSON 분기)이 존재한다. Actuator 헬스체크(container.md/graceful-shutdown.md 참고)와 Micrometer-Prometheus 메트릭(`io.micrometer:micrometer-registry-prometheus` + `GET /actuator/prometheus`)이 모두 구현되어 있다. Micrometer Tracing(OpenTelemetry 연동)은 이 저장소 규모에서 아직 필요하지 않다고 판단해 도입하지 않았다. |
...
**요약**: 24개 root 아키텍처 문서 중 **Covered 22 / Thin 1 / Missing 1**. `notification`은 최상위 공유 모듈이 아니라 `account` 도메인 내부(Technical Service)에 배치되어 있다.
