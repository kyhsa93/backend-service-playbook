...
| [domain-events.md](../architecture/domain-events.md) | **Covered** | `docs/architecture/domain-events.md`, `examples/.../outbox/OutboxEvent.java`, `OutboxWriter.java`, `OutboxRelay.java`, `examples/.../account/application/event/*EventHandler.java` | Outbox 테이블(JPA `@Entity`) + `AccountRepositoryImpl.save()`가 Aggregate와 같은 트랜잭션에서 이벤트를 저장 + Command Service가 저장 직후 `OutboxRelay.processPending()`을 동기 호출하는 경로가 구현되어 있다. root/nestjs의 `@Scheduled` 폴링 + 메시지 큐 대신 동기 드레인을 쓰는 의도적 차이가 있다(문서에 명시). |
...
| [observability.md](../architecture/observability.md) | **Covered (코드는 일부 gap)** | `docs/architecture/observability.md` | 구조화 로깅(Logback JSON 인코더 + `StructuredArguments.kv`), MDC 기반 Correlation ID, Actuator/Micrometer 메트릭을 정의. `NotificationServiceImpl`/`OutboxRelay`/`AccountController` 모두 `StructuredArguments.kv(...)`로 구조화 로깅을 하고, `logback-spring.xml`(로컬 평문/운영 JSON 분기)이 존재한다. Actuator 헬스체크는 도입되어 있다(위 container.md/graceful-shutdown.md 참고) — 남은 gap은 Micrometer 기반 커스텀 메트릭·트레이싱 미도입뿐이다. |
| [scheduling.md](../architecture/scheduling.md) | **Covered** | `docs/architecture/scheduling.md` | `@Scheduled`/`@EnableScheduling`, Task Outbox, FIFO dedup·ShedLock 기반 다중 인스턴스 안전성을 정의. `@Scheduled` 작업 자체는 아직 없다(배치 유스케이스 부재) — 문서는 향후 domain-events.md의 Outbox Relay 구현 시 참조용으로 작성됨. |
