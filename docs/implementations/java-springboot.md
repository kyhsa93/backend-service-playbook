...
| [domain-events.md](../architecture/domain-events.md) | **Covered (코드는 gap)** | `docs/architecture/domain-events.md` | Outbox 테이블 스키마, `OutboxRelay`(`@Scheduled`), `EventConsumer`, 멱등성 3단계를 상세 기술. **현재는 `ApplicationEventPublisher`/`@EventListener` 기반 동기 in-process 발행** — root가 명시적으로 요구하는 Outbox 경로가 전혀 없음을 문서에서 강조. |
...
**요약**: 24개 root 아키텍처 문서 중 **Covered 21 (그중 코드 gap이 남아있는 항목 15) / Thin 1 / Missing 2**. 이전 감사(Covered 4 / Thin 9 / Missing 11) 대비 문서 커버리지가 크게 확장되었으나, **문서화 ≠ 코드 반영**이라는 점이 중요하다 — 표의 "코드는 gap" 표시가 붙은 15개 항목은 `examples/`를 아직 고쳐야 문서와 코드가 일치한다.
