...
| [directory-structure.md](../architecture/directory-structure.md) | **Covered** | `docs/architecture/directory-structure.md` | Account 모듈 실제 트리 전체를 문서화(notification은 account 내부 Technical Service로 재배치됨). `interfaces/rest/`(복수형, Java 예약어 `interface` 회피)를 root의 `interface/`(단수)와 명시적으로 대조. |
| [repository-pattern.md](../architecture/repository-pattern.md) | **Covered** | `docs/architecture/repository-pattern.md` | 올바른 `find<Noun>s`/`save<Noun>`/`delete<Noun>` 컨벤션과 `take:1` 단건 조회 패턴을 정의. **`AccountRepository`에 `delete(String accountId)`(soft delete)가 추가되어 root의 "delete 메서드 필요" 원칙과 일치한다.** **`findByAccountIdAndOwnerId`(단건 전용 메서드)/`findAll`+`countAll`을 `findAccounts`(목록+count를 `AccountsWithCount`로 함께 반환) 하나로 병합했고, `save`는 `saveAccount`로 리네이밍했다 — 단건 조회는 `findAccounts`를 `take:1`로 호출한 뒤 `Stream.findFirst()`로 통일된다. 하위 Entity `Transaction`의 `findTransactions`/`countTransactions`도 동일하게 `findTransactions` 하나로 병합됨(#184)** — 더 이상 gap이 아니다. |
...
| [error-handling.md](../architecture/error-handling.md) | **Covered** | `docs/architecture/error-handling.md` | `AccountException.ErrorCode` enum, `ErrorResponse{statusCode,code,message,error}` 4필드 구조, `@RestControllerAdvice` 전역 Validation 처리를 정의. **`ErrorResponse`가 이제 `statusCode`/`code`/`message`/`error` 4필드를 모두 갖고, `AccountController.handleAccountException`이 `ErrorResponse.of(status, code, message)`로 이를 채운다** — 더 이상 gap이 아니다. 계층별 예외 throw/변환 분리 자체는 이미 올바름. |
| [persistence.md](../architecture/persistence.md) | **Covered** | `docs/architecture/persistence.md` | `@Transactional`/`REQUIRES_NEW`(이미 `NotificationServiceImpl`에서 사용 중) 전파, Flyway 마이그레이션, soft delete 배선을 정의. **마이그레이션은 Flyway(`db/migration/`) + `ddl-auto: validate`로 이미 해결됨.** **`Account.delete()`(도메인, CLOSED 상태만 허용) + `AccountRepository.delete()`(soft delete) + `DeleteAccountService`(application, 소유권 검증 후 호출)로 soft delete가 실제로 배선되어 더 이상 gap이 아니다.** |
...
| [container.md](../architecture/container.md) | **Covered** | `docs/architecture/container.md`, `examples/Dockerfile` | Layered JAR 기반 Gradle 멀티스테이지 Dockerfile, JRE 베이스, Actuator 헬스체크를 정의. **멀티스테이지 Dockerfile(Layered JAR, JRE 베이스, non-root 사용자)과 Actuator 헬스체크(`spring-boot-starter-actuator` + `management.endpoint.health.probes.enabled`) 모두 이미 적용됨** — 코드 gap 없음. |
| [graceful-shutdown.md](../architecture/graceful-shutdown.md) | **Covered** | `docs/architecture/graceful-shutdown.md` | `server.shutdown: graceful`, Actuator liveness/readiness, `AvailabilityChangeEvent` 자동 연동을 정의. **`application.yml`에 `server.shutdown: graceful` + `timeout-per-shutdown-phase: 30s`, Actuator liveness/readiness probes 모두 이미 설정되어 있다** — 더 이상 gap이 아니다. |
| [observability.md](../architecture/observability.md) | **Covered (코드는 일부 gap)** | `docs/architecture/observability.md` | 구조화 로깅(Logback JSON 인코더 + `StructuredArguments.kv`), MDC 기반 Correlation ID, Actuator/Micrometer 메트릭을 정의. **`NotificationServiceImpl`/`OutboxRelay`/`AccountController` 모두 이미 `StructuredArguments.kv(...)`로 구조화 로깅을 하고, `logback-spring.xml`(로컬 평문/운영 JSON 분기)도 이미 존재하며, `AccountController`의 예외 핸들러도 이미 `log.warn(...)`을 남긴다** — 더 이상 "비구조화 텍스트 로그"가 아니다. Actuator 헬스체크는 이미 도입됨(위 container.md/graceful-shutdown.md 참고) — 남은 gap은 Micrometer 기반 커스텀 메트릭·트레이싱 미도입뿐이다. |
...
**요약**: 24개 root 아키텍처 문서 중 **Covered 21 (그중 완전히 해소된 항목 19, 일부만 남은 항목 2, 코드 gap이 여전히 남아있는 항목 0 — 어떤 형태로든 gap이 있는 항목 합계 2) / Thin 1 / Missing 2**. 이전 감사(Covered 4 / Thin 9 / Missing 11) 대비 문서 커버리지가 크게 확장되었고, JWT 인증·Correlation ID/요청 로깅·Secrets Manager·aggregate-id·테스트 피라미드 5개 항목은 이전 세션에서 코드가 문서를 따라잡아 gap 목록에서 완전히 빠졌다(`domain-events.md`는 그보다 앞서 이미 빠졌다). 이후 세션에서는 `layer-architecture`(Domain/JPA 분리 — `AccountJpaEntity`/`TransactionJpaEntity`/`MoneyEmbeddable` + `AccountMapper`/`TransactionMapper` 도입), `error-handling`(`ErrorResponse` 4필드화), `cqrs-pattern`(`GetTransactionsService`도 `AccountQuery`로 전환 + `AccountQueryRepository` → `AccountQuery` 리네이밍 + harness `cqrs-query-purity` 규칙 추가), `repository-pattern`(`AccountRepository.delete()` soft delete 추가 + 조회/저장 메서드 root 네이밍 통일), `graceful-shutdown`/`container`(Actuator + `server.shutdown: graceful` 도입), `authentication`(`Credential`/bcrypt 기반 실제 비밀번호 검증 + `POST /auth/sign-up` 신설, issue #189)이 이후 라운드에서 완전히 해소되었다(이 문서의 해당 서술 갱신은 issue #196). `persistence`도 soft delete 배선까지 완전히 해소되었다. 남은 gap은 `config`(AwsProperties 자격증명 필드 Bean Validation 미적용, 세분화된 설정 파일 분리 미제안 이상)와 `observability`(Micrometer 기반 커스텀 메트릭·트레이싱 미도입) 2개뿐이다. `notification`은 최상위 공유 모듈에서 `account` 도메인 내부(Technical Service)로 재배치되었다.
...
| 문서 | 내용 |
|---|---|
| `docs/architecture/bootstrap.md` | `AccountServiceApplication`의 `@SpringBootApplication`/`SpringApplication.run()` 부트스트랩 순서, `application.yml` 로딩 순서, `@RestControllerAdvice` 전역 예외 처리 배선(error-handling.md 교차 참조). Actuator는 이미 도입됨(이 문서 갱신은 issue #196) — springdoc/CORS 도입 가이드만 여전히 forward-looking |
...
| `docs/architecture/design-principles.md` | 이 저장소의 핵심 설계 원칙 13개를 압축한 TL;DR 인덱스 + 남은 gap 요약 표. `GetTransactionsService`의 Query 인터페이스 전환은 이미 완료됨(`AccountQuery` 의존, issue #196에서 이 표의 stale 서술 정정) — 문서 자체에 남은 gap은 config.md의 Bean Validation 미대상 필드뿐 |
...
| `docs/architecture/rate-limiting.md` | Resilience4j `RateLimiter` 기반 `Filter` 구현 예시. `build.gradle`에 `resilience4j-spring-boot3` 의존성이 있고 `RateLimitFilter`가 실제로 모든 비-actuator 요청에 적용된다(이 문서 갱신은 issue #196) — 더 이상 forward-looking이 아니다 |
...
- `package-structure`는 디렉토리 존재 여부만 확인 — `cqrs-pattern.md`가 명시한 "Query Service가 Repository를 직접 쓰면 안 된다" 위반(`GetTransactionsService`에 남은 gap)은 감지하지 못한다.
- `domain-purity`는 Spring 스테레오타입 애노테이션만 금지하고 **JPA 애노테이션은 검사 대상이 아니다** — `Account`/`Transaction`/`Money`가 순수 도메인으로 분리된 지금도, 만약 누군가 다시 `@Entity`를 domain 클래스에 붙이는 회귀가 생기더라도 이 harness는 잡아내지 못한다.
