...
| [domain-events.md](../architecture/domain-events.md) | `docs/architecture/domain-events.md`, `examples/src/outbox/`, `examples/.../application/event/*_event_handler.py`, `examples/.../application/integration_event/` | 완전 | 이벤트 수집(`_events`/`pull_events()`), Outbox 적재(Repository `save()`가 같은 세션에서 `OutboxWriter.save_all()` 호출)까지는 그대로다. 드레인 경로는 독립적으로 주기 실행되는 `OutboxPoller`(Outbox → SQS 발행, 1초 tick)와 `OutboxConsumer`(SQS → EventHandler 수신, long polling)가 전담한다 — Command Handler는 저장 직후 동기 호출로 관여하지 않는다. Account→Card Integration Event(`account.suspended.v1`/`account.closed.v1`)도 이 경로를 그대로 탄다. 이벤트 핸들러 멱등성도 Level 2(Ledger) 그대로 유지됨 — 이제는 `OutboxPoller`가 SQS 발행 시점에 `outbox_event_id`를 payload에 심어 넣고, `OutboxConsumer`가 이를 그대로 핸들러에 전달해 `SentEmailModel.outbox_event_id`에 기록, 재전달 시 중복 발송을 건너뛴다 |
| [cqrs-pattern.md](../architecture/cqrs-pattern.md) | `docs/architecture/cqrs-pattern.md`, `examples/.../application/command 또는 query/*_handler.py` | 완전 | `XxxHandler` + `async def execute()`, `@dataclass` Command/Query는 코드와 문서가 일치. Bus 도입 기준과 트레이드오프도 문서화. CommandBus/QueryBus 생략은 루트도 선택사항으로 명시하므로 격차 아님 |
...
| [local-dev.md](../architecture/local-dev.md) | `docs/architecture/local-dev.md`, `examples/docker-compose.yml`, `examples/.env.example`, `examples/localstack/init-ses.sh`, `examples/localstack/init-sqs.sh` | 완전 | Postgres + LocalStack(SES, Secrets Manager, SQS) 조합, healthcheck, 버전 고정 LocalStack 이미지, `app` 서비스(`profiles: [app]`), `.env.example`/`.env.development` 모두 실제 코드로 존재. SQS는 `OutboxPoller`/`OutboxConsumer`가 쓰는 `domain-events`/`domain-events-dlq` 큐를 `init-sqs.sh`가 생성한다(nestjs 구현과 큐 이름·RedrivePolicy 동일) — 코드와 문서가 일치 |
