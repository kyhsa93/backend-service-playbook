...
| [authentication.md](../architecture/authentication.md) | `docs/architecture/authentication.md` | 완전 | **코드 격차 (문서에 명시)**: 현재 라우터는 `X-User-Id` 헤더를 검증 없이 신뢰한다 — 인증이 아니라 신뢰 기반 헤더 전달이다. 문서가 `python-jose` 기반 JWT 발급/검증, `HTTPBearer` + `Depends(get_current_user)` 패턴, 라우터 레벨 일괄 적용까지 구체적인 구현으로 제시 |
...
| [config.md](../architecture/config.md) | `docs/architecture/config.md`, `examples/src/database.py` (`os.getenv`) | 완전 | **코드 격차 (문서에 명시)**: fail-fast 검증, 관심사별 설정 분리가 없다. `DATABASE_URL`을 `os.getenv` 기본값으로만 처리 — 문서가 `pydantic-settings`의 `BaseSettings` 기반 관심사별 설정 클래스와 `validate_env()` fail-fast 패턴을 구체적으로 제시 |
| [secret-manager.md](../architecture/secret-manager.md) | `docs/architecture/secret-manager.md` | 완전 | **코드 격차 (문서에 명시)**: `SecretService` 추상화, TTL 캐시가 없다 — AWS 자격증명은 `os.getenv(..., "test")` 하드코딩 기본값뿐. 문서가 `aioboto3` 기반 `AwsSecretService` + TTL 캐시 구현을 구체적으로 제시 |
