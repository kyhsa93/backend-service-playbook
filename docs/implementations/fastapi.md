...
| [domain-events.md](../architecture/domain-events.md) | `docs/architecture/domain-events.md`, `examples/src/outbox/`, `examples/.../application/event/*_event_handler.py`, `examples/.../application/integration_event/` | 완전 | 이벤트 수집(`_events`/`pull_events()`), Outbox 적재(Repository `save()`가 같은 세션에서 `OutboxWriter.save_all()` 호출)까지는 그대로다. 2026-07 async 전환으로 드레인 경로가 바뀌었다 — Command Handler가 저장 직후 동기 호출하던 `OutboxRelay.process_pending()`을 완전히 제거하고, 독립적으로 주기 실행되는 `OutboxPoller`(Outbox → SQS 발행, 1초 tick)와 `OutboxConsumer`(SQS → EventHandler 수신, long polling)로 교체했다. Account→Card Integration Event(`account.suspended.v1`/`account.closed.v1`)도 이 경로를 그대로 탄다. 이벤트 핸들러 멱등성도 Level 2(Ledger) 그대로 유지됨 — 이제는 `OutboxPoller`가 SQS 발행 시점에 `outbox_event_id`를 payload에 심어 넣고, `OutboxConsumer`가 이를 그대로 핸들러에 전달해 `SentEmailModel.outbox_event_id`에 기록, 재전달 시 중복 발송을 건너뛴다 |
