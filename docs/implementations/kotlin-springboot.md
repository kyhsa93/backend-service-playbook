...
| [local-dev.md](../architecture/local-dev.md) | **Covered** | `docs/architecture/local-dev.md`, `examples/docker-compose.yml`, `examples/localstack/init-ses.sh` | Postgres + LocalStack(SES) healthcheck 구성이 정의와 일치한다. `app` 서비스도 `profiles: [app]`으로 존재하고, `.env.example`/`.env.development`도 있다 — 기본 흐름은 여전히 호스트에서 `./gradlew bootRun`이지만 컨테이너 실행 옵션도 갖췄다. |
