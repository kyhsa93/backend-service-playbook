...
| [persistence.md](../architecture/persistence.md) | **Covered (코드는 갭)** | `docs/architecture/persistence.md` | `@Transactional` 전파, Flyway 마이그레이션(`ddl-auto: validate` + `db/migration/V1~V4`)은 이미 적용 완료 — 더 이상 갭 아님. **남은 갭은 Soft Delete 배선뿐이다**: `deletedAt` 컬럼은 있지만 `AccountRepository`에 delete 메서드 자체가 없어 실행 경로가 없다. |
...
**요약**: 24개 root 아키텍처 문서 + `conventions.md` 총 25개 중 **Covered 21 (그중 코드 갭이 남아있는 항목 13) / Thin 2 / Missing 2**. 이전 감사(Covered 5 / Thin 9 / Missing 11) 대비 문서 커버리지가 크게 확장되었으나, **문서화 ≠ 코드 반영**이라는 점이 중요하다 — 표의 "코드는 갭" 표시가 붙은 13개 항목은 `examples/`를 아직 고쳐야 문서와 코드가 일치한다. (domain-events.md는 이번에 Outbox 패턴이 실제로 구현되어 이 목록에서 빠졌다.)
