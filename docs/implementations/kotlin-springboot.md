...
| [domain-events.md](../architecture/domain-events.md) | **Covered** | `docs/architecture/domain-events.md`, `examples/.../outbox/`, `examples/.../account/application/event/*EventHandler.kt` | Outbox 테이블(`OutboxEvent`)에 Aggregate와 같은 트랜잭션으로 이벤트를 커밋한다. Command Service는 저장 후 곧바로 반환하고, `OutboxPoller`(`@Scheduled(fixedDelay = 1000)`)가 독립적으로 테이블을 SQS로 발행하며 `OutboxConsumer`(SQS long polling)가 `EventHandlerRegistry`를 통해 핸들러를 호출한다(2026-07 async 전환 — 이전의 동기 드레인 `OutboxRelay`는 삭제됨). 6개 Domain Event가 공통 `sealed interface DomainEvent`를 구현하고, Handler는 Outbox 행의 `eventId`를 `sourceEventId`로 전달해 `sent_emails.source_event_id`(unique)로 Level 2(Ledger) 중복 발송 방지를 적용한다. |
...
| [scheduling.md](../architecture/scheduling.md) | **Covered** | `docs/architecture/scheduling.md`, `examples/.../outbox/{OutboxPoller,OutboxConsumer}.kt` | `@Scheduled`, Task Outbox, 코루틴을 사용하지 않는 이유(블로킹 MVC+JPA 스택)를 정의. `OutboxPoller`(`@Scheduled(fixedDelay = 1000)`)가 실제 코드로 존재하고, `OutboxConsumer`는 `SmartLifecycle` 기반 전용 스레드로 SQS를 long polling한다(코루틴 미도입 결정과 일관). |
...
| [local-dev.md](../architecture/local-dev.md) | **Covered** | `docs/architecture/local-dev.md`, `examples/docker-compose.yml`, `examples/localstack/{init-ses.sh,init-secrets.sh,init-sqs.sh}` | Postgres + LocalStack(SES/Secrets Manager/SQS) healthcheck 구성이 정의와 일치한다. SQS는 `domain-events`/`domain-events-dlq` 큐(RedrivePolicy `maxReceiveCount=3`)를 `init-sqs.sh`가 생성한다. `app` 서비스도 `profiles: [app]`으로 존재하고, `.env.example`/`.env.development`도 있다 — 기본 흐름은 여전히 호스트에서 `./gradlew bootRun`이지만 컨테이너 실행 옵션도 갖췄다. |
...
| **[domain-service.md](../architecture/domain-service.md)** | **Covered** | `implementations/kotlin-springboot/docs/architecture/domain-service.md`, `examples/.../payment/domain/{Payment.kt, Refund.kt, RefundEligibilityService.kt}` | Technical Service 패턴(`NotificationService`)에 이어, Payment BC(3번째 도메인, Payment+Refund 두 Aggregate)의 `RefundEligibilityService`가 진짜 cross-Aggregate 조율 예시다 — 어노테이션 없는 순수 클래스, `RequestRefundService`가 두 Repository를 로드해 위임, 거부는 예외가 아니라 `REJECTED` + 201로 응답. Kotlin 전용 전담 문서(`domain-service.md`)가 실제 코드를 인용해 신설되었다. |
| [cross-domain-communication.md](../architecture/cross-domain-communication.md) | **Covered** | Kotlin 전용 대응 문서는 [`cross-domain.md`](../../implementations/kotlin-springboot/docs/architecture/cross-domain.md)(아래 "Kotlin Spring Boot 전용" 절) | Account/Card/Payment 세 BC가 실제로 존재해 동기 Adapter(`CardAdapter`/`AccountAdapter`)와 비동기 Integration Event(`account.suspended.v1`/`account.closed.v1`/`payment.completed.v1`/`payment.cancelled.v1`/`refund.approved.v1`) 양쪽 모두 실제 코드로 구현되어 있다. |
...
**요약**: 24개 root 아키텍처 문서 + `conventions.md` 총 25개 중 **Covered 23 / Thin 1 / Missing 1**. **문서화 ≠ 코드 반영**이라는 점이 중요하다 — 표의 "코드는 갭" 표시가 붙은 항목은 `examples/`를 아직 고쳐야 문서와 코드가 일치한다. domain-events.md의 이전 잔여 갭(`sealed interface` 이벤트 계층, Handler Level 2 멱등성)은 모두 코드에 반영되었다. domain-service.md는 Payment BC(`RefundEligibilityService`) 추가로 Thin에서 Covered로 전환되었다.
...
| — (Kotlin Spring Boot 전용) | `implementations/kotlin-springboot/docs/architecture/cross-domain.md` — Adapter 패턴 구현 상세. Account/Card 두 BC가 실제로 존재해 동기 Adapter와 비동기 Integration Event 양쪽 모두 실제 코드로 다룬다. 원칙은 [cross-domain-communication.md](../architecture/cross-domain-communication.md) 참고 |
