...
| [repository-pattern.md](../architecture/repository-pattern.md) | **Covered** | `docs/architecture/repository-pattern.md` | 올바른 `find<Noun>s`/`save<Noun>`/`delete<Noun>` 컨벤션과 `take:1` 단건 조회 패턴을 정의. **`delete<Noun>`(`deleteAccount`)는 실제 코드에 추가 완료** — `Account.markDeleted()`가 CLOSED 상태만 삭제 가능하도록 강제한다. **`AccountRepository.findAccounts`/`saveAccount`/`findTransactions`로 이미 리네이밍되어 있다(issue #185)** — 더 이상 갭이 아니다. |
...
| [config.md](../architecture/config.md) | **Covered** | `docs/architecture/config.md`, `examples/.../config/{AwsProperties,SesProperties,JwtProperties}.kt` | `aws`/`ses`/`jwt` 네임스페이스 모두 `@ConfigurationProperties` + `@Validated`로 관심사별 분리와 Fail-fast를 갖춘다. **`jwt.secret`도 이제 `JwtProperties`(`@ConfigurationProperties`)로 전환되어 더 이상 `@Value` 기반이 아니다(issue #136)** — 더 이상 갭이 아니다. |
| [container.md](../architecture/container.md) | **Covered** | `docs/architecture/container.md`, `examples/Dockerfile` | Gradle 멀티스테이지 Dockerfile, JRE 베이스, non-root, exec form CMD가 문서 예시와 동일하게 실제로 존재. **Actuator 기반 liveness/readiness 프로브도 `build.gradle.kts`의 `spring-boot-starter-actuator` 의존성으로 이미 적용됨** — 더 이상 갭이 아니다. |
| [graceful-shutdown.md](../architecture/graceful-shutdown.md) | **Covered** | `docs/architecture/graceful-shutdown.md` | `server.shutdown: graceful`, Actuator liveness/readiness, `ThreadPoolTaskScheduler` 종료 대기를 정의. **`application.yml`에 `server.shutdown: graceful` + Actuator liveness/readiness probes가 이미 설정되어 있다** — 더 이상 갭이 아니다. |
...
| **[conventions.md](../conventions.md)** | **Thin** | `examples/.../interfaces/rest/AccountController.kt` | REST URL 설계, HTTP 상태 코드 구분은 root와 일치. **Rate Limiting은 이제 `RateLimitingFilter` + `resilience4j-spring-boot3`로 실제 구현되어 있다** — 아래 "Kotlin Spring Boot 전용" 절의 [`rate-limiting.md`](../../implementations/kotlin-springboot/docs/architecture/rate-limiting.md) 참고. Repository 메서드 네이밍은 [repository-pattern.md](../architecture/repository-pattern.md) 행대로 이미 해소됨. |

**요약**: 24개 root 아키텍처 문서 + `conventions.md` 총 25개 중 **Covered 21 (그중 코드 갭이 남아있는 항목 7) / Thin 2 / Missing 2**. 이전 감사(Covered 5 / Thin 9 / Missing 11) 대비 문서 커버리지가 크게 확장되었으나, **문서화 ≠ 코드 반영**이라는 점이 중요하다 — 표의 "코드는 갭" 표시가 붙은 항목은 `examples/`를 아직 고쳐야 문서와 코드가 일치한다. (domain-events.md는 Outbox 패턴이 실제로 구현되어 이 목록에서 빠졌고, 이번 변경으로 error-handling.md·persistence.md도 코드 갭이 해소되어 빠졌다. 이후 세션에서 `repository-pattern`(`find<Noun>s`/`save<Noun>` 리네이밍, issue #185), `container`/`graceful-shutdown`(Actuator + `server.shutdown: graceful` 도입), `config`(`jwt.secret`도 `@ConfigurationProperties`로 전환, issue #136)가 추가로 해소되어 목록에서 빠졌다 — 이 문서의 해당 서술 갱신은 issue #197. Rate Limiting(conventions.md 행, `rate-limiting.md` 보너스 문서 행)도 실제로 구현되어 더 이상 미구현이 아니다.)
...
| — (Kotlin Spring Boot 전용) | `implementations/kotlin-springboot/docs/architecture/rate-limiting.md` — Resilience4j `RateLimiter` 기반 Filter. `build.gradle.kts`에 `resilience4j-spring-boot3` 의존성이 있고 `RateLimitingFilter`가 실제로 적용되어 있다(issue #197) — 더 이상 미구현이 아니다 |
