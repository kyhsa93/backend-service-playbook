...
**요약**: 24개 root 아키텍처 문서 + `conventions.md` 총 25개 중 **Covered 22 / Thin 2 / Missing 1**. **문서화 ≠ 코드 반영**이라는 점이 중요하다 — 표의 "코드는 갭" 표시가 붙은 항목은 `examples/`를 아직 고쳐야 문서와 코드가 일치한다. domain-events.md의 잔여 갭(`sealed interface` 이벤트 계층, Handler Level 2 멱등성) 정도가 현재 남아있는 코드 갭이다.
