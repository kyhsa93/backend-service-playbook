...
**이후 갱신** (2026-07-10~11 세션): JWT 인증, 테스트 피라미드, Dockerfile, aggregate-id 하이픈 제거, 구조화 로깅/Correlation ID, Secrets Manager 연동, 환경 변수 fail-fast 검증, local-dev `.env`/`app` 서비스가 모두 실제로 구현되었다(이슈 #47-98). 아래는 **여전히 코드에 남아있는 격차**다:

1. **persistence.md**: 컨텍스트 기반 여러 Repository 트랜잭션 전파 미구현, 마이그레이션 롤백 스크립트 부재.
2. **error-handling.md**: 표준 JSON 에러 응답 스키마(`statusCode`/`code`/`message`/`error`) 미구현, 평문 텍스트만 반환.
3. **graceful-shutdown.md**: `signal.NotifyContext` + `Shutdown(ctx)` 미구현 — `http.ListenAndServe` 블로킹 호출 그대로.
4. **file-storage.md**, **scheduling.md**: 해당 패턴 자체가 `examples/`에 전혀 구현되어 있지 않음 — 문서는 전량 목표 설계.
5. **layer-architecture.md**: Query Handler가 별도 Query 인터페이스 없이 Command와 동일한 `account.Repository` 재사용.
6. **repository-pattern.md**: `FindByID`/`FindAll` 분리, `Delete<Noun>` 부재.
