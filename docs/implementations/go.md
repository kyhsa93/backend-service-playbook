...
**→ [implementations/go/docs/architecture/](../../implementations/go/docs/architecture/)** — root `docs/architecture/*` 24개 문서 중 24개 각각에 대응하는 Go 구현 상세 21개 문서(전략적 설계는 언어 무관이라 root를 그대로 참조, domain-service는 root 문서가 nestjs 코드를 실제 예시로 인용하고 있어 Go도 별도 문서 없이 root를 참조하되 아래 표에서 Go 자신의 코드를 인용, cross-domain-communication은 [`cross-domain.md`](../../implementations/go/docs/architecture/cross-domain.md)가 대응)
**→ [implementations/go/examples/](../../implementations/go/examples/)** — Account/Card/Payment 세 Bounded Context의 전체 구현 예시 (계좌 개설/입출금/정지/재개/종료 + 이메일 알림, 카드 발급/정지/해지, 카드결제·결제취소·환불)
...
| [domain-events.md](../architecture/domain-events.md) | Covered | [`domain-events.md`](../../implementations/go/docs/architecture/domain-events.md) — Aggregate의 이벤트 수집(`events []DomainEvent`)과 Outbox 패턴(적재 + 비동기 발행/수신)이 모두 실제로 구현되어 있음을 코드 기준으로 설명. `internal/infrastructure/persistence/account_repository.go`의 `Save`가 계좌/거래 row와 outbox row를 같은 트랜잭션에 커밋하고, Command Handler는 그 즉시 반환한다 — 이후 `internal/infrastructure/outbox/poller.go`의 `Poller.Run`이 1초 tick마다 독립적으로 실행되며 미처리 outbox 행을 SQS로 발행하고, `consumer.go`의 `Consumer.Run`이 SQS를 long polling해 핸들러를 실행한다(2026-07 async 전환, nestjs 구현체와 동일한 전략 — 동기 드레인 완전 제거). Poller 발행 실패 행은 `processed=false`로 남아 다음 tick에 재시도되고, Consumer 핸들러 실패는 메시지를 삭제하지 않아 SQS visibility timeout 이후 재전달된다(at-least-once). |
...
| [observability.md](../architecture/observability.md) | Covered | [`observability.md`](../../implementations/go/docs/architecture/observability.md) — `log/slog` 구조화 로깅(JSON, snake_case 필드), 레이어별 로깅 기준, `context.Context` 기반 Correlation ID 전파를 실제 코드로 설명. `notification/service.go`/`outbox/consumer.go`가 `slog`를 쓰고, `middleware/correlation_id_middleware.go`가 Correlation ID를 전파한다. `internal/infrastructure/logging.CorrelationHandler`가 `slog.Handler`를 감싸 모든 로그 레코드에 correlation_id를 자동 주입한다(`main.go`의 로거 초기화에 적용) — 호출부마다 수동으로 넘길 필요가 없다. |
...
| [domain-service.md](../architecture/domain-service.md) | Covered | 루트 문서가 이미 nestjs의 `RefundEligibilityService`를 실제 동작하는 cross-Aggregate 예시로 인용하고 있어, Go는 별도의 전용 문서를 두지 않고(nestjs와 동일한 판단) 그 자리를 채우는 Go 코드만 여기서 인용한다 — `internal/domain/payment/refund_eligibility_service.go`의 `EvaluateRefundEligibility(payment *Payment, refund *Refund) RefundDecision`이 Payment BC 추가로 실제로 동작하는 두 번째 언어 구현이다. "원 결제가 COMPLETED 상태여야 하고 환불 금액이 결제 금액을 넘을 수 없다"는 판단은 `Payment`도 `Refund`도 혼자 내릴 수 없으므로(각자 상대 Aggregate의 상태/금액을 모른다) `RequestRefundHandler`가 두 Repository로 각각 로드한 뒤 이 함수에 위임한다. 프레임워크 데코레이터가 없는 평범한 패키지 함수로 표현한 것은 Go 고유의 선택이다 — Go에는 DI 컨테이너가 없고 판단 자체가 상태를 갖지 않으므로, 상태 없는 구조체+메서드보다 자유 함수가 더 idiomatic하다(nestjs는 언어 관용상 `new`로 직접 인스턴스화하는 클래스를 쓴다). Technical Service(`command.Notifier` ← `notification.Service`)는 여전히 [domain-events.md](../../implementations/go/docs/architecture/domain-events.md)/[secret-manager.md](../../implementations/go/docs/architecture/secret-manager.md)/[file-storage.md](../../implementations/go/docs/architecture/file-storage.md)에서 개별적으로 다룬다. |
