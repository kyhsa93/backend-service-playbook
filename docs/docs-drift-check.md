...
이 두 패턴은 2026-07-18 문서 감사에서 반복적으로 발견됐다(예: Card BC가 이미 4개 언어에 구현됐는데 root 커버리지 문서는 "단일 도메인이라 Missing"이라고 서술, `AwsProperties.accessKeyId`가 이미 Bean Validation 대상인데 문서는 "미적용"이라고 서술 등). 사람이 매번 grep으로 찾는 대신 CI에서 상시 점검한다.
