...
리소스 경합(5개 언어 testcontainers를 동시 실행하면 이 머신의 Docker 데몬이 과부하로
응답 불능 상태에 빠질 수 있다)을 피하려 2개씩 파도 형태로 실행했다(go+fastapi →
nestjs+java-springboot → kotlin-springboot 단독).
