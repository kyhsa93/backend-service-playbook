...
**1 Bounded Context = 1 독립 모듈/서비스**. BC 경계가 코드 모듈 경계다.

```
주문 BC   → OrderModule   (src/order/ 또는 order-service/)
사용자 BC → UserModule    (src/user/ 또는 user-service/)
결제 BC   → PaymentModule (src/payment/ 또는 payment-service/)
```

각 BC 내부 구현(Aggregate, Repository, Service)은 외부에 노출하지 않는다. 다른 BC는 공개된 인터페이스(exported service, Integration Event)만 사용한다.
...
BC 간의 관계 유형을 정의한다. 관계 유형에 따라 구현 방식이 결정된다.

#### 관계 유형과 구현 방식

| 패턴 | 설명 | 구현 방식 |
|------|------|----------|
| **ACL** (Anticorruption Layer) | 하류 BC가 상류 모델의 오염을 방지하기 위해 변환 계층을 둠 | Adapter 패턴 — 외부 모델을 내부 도메인 모델로 변환 |
...
| **Shared Kernel** | 두 BC가 일부 모델을 공유 | 공유 모듈(`shared/`). 변경 시 양측 합의 필요. 최소화 권장 |
...
order/
  ...
  infrastructure/user-adapter-impl.ts          ← 구현체
```
...
전략적 설계 산출물(Subdomain 분류표, BC 정의서, Context Map)을 합의한 후 전술 설계로 진입한다.
